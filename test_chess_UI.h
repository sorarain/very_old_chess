void draw_Chess_Line(HANDLE handleOut,DWORD a)
{
    ...
    for(int i = 1;i < 15;i++)
    {
        ...
        for(int j = 1;j <= 15;j++)
        {
            WriteConsoleOutputCharacter(handleOut,"£ü",2,pos_paint_now_1,&a);
            WriteConsoleOutputCharacter(handleOut,"£ü",2,pos_paint_now_2,&a);
...
//     WriteConsoleOutputCharacter(handleOut,"µ¥»÷Êó±ê×ó¼üÂä×Ó",16,pos_Text,&a);
// }
void load_UI_Tips(HANDLE handleOut,const char* s,int length)
{
    DWORD a;
    COORD pos_Text;
    pos_Text.X = PAINT_Y;
    pos_Text.Y = PAINT_X - 3;
    WriteConsoleOutputCharacter(handleOut,s,length,pos_Text,&a);
}
...
//		{
//			FILE *fin;
//    		fin = fopen("chess_test.txt","wb");
//    		for(int i = 1;i <= CHESS_HEIGHT;i++)
//    		{
//    			for(int j = 1;j <= CHESS_LENGTH;j++)
//    			{
//    				fprintf(fin,"%d ",board[i][j]);
//				}
//				fprintf(fin,"\n");
//			}
//			fclose(fin);
...
//		}
    ...
    writeChessPiece(pos_Chess.X,pos_Chess.Y,handleOut,"¡ñ");
    board[pos_Chess.X][pos_Chess.Y] = C_BLACK;
    Points player_Piece;//
    player_Piece.x = pos_Chess.X;//
    player_Piece.y = pos_Chess.Y;//
    q[++R] = player_Piece;//
}
